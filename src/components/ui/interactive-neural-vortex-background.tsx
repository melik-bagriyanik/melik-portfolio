...
export const InteractiveNeuralVortex = ({ className, opacity = 0.6 }: Props) => {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        color = vec3(0.03, 0.08, 0.28);
        // Mix toward a cooler blue accent based on scroll
        color = mix(color, vec3(0.08, 0.25, 0.7), 0.35 + 0.18 * sin(2.0 * u_scroll_progress + 1.2));
        // Additive royal-blue glow
        color += vec3(0.05, 0.15, 0.55) * sin(2.0 * u_scroll_progress + 1.5);
        color = color * noise;
